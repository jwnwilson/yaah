...
> **Orchestration direction (see [ADR-0002](docs/adr/0002-lead-driven-orchestration.md), 2026-06-14):** the team lead is becoming a real **orchestrator agent** — it dispatches other agents and triggers a completion monitor — rather than the fixed `PLAN→IMPLEMENT→VERIFY` pipeline. Agents run as **Temporal child-workflow actors** with signal-fed mailboxes, talk to each other live mid-run, and drain their inbox until empty; **Temporal stays the durable executor** (orchestrator-worker pattern). This supersedes the original "workflow is the sole supervisor" stance in the design spec. Build order: orchestration foundation spec first, then the agent-visibility UI (team roster, per-agent output, message inbox, ticket assignee) as a second spec on top. ADRs live in `docs/adr/`.
